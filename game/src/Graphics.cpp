...
{
	RLAPI::DrawCircleLines(static_cast<int>(pos.x), static_cast<int>(pos.y), static_cast<float>(radius), color);
...
void Graphics::DrawFillCircle(const Vec2 &pos, const float radius, const Color &color)
{
	...
	RLAPI::DrawCircleV(center, static_cast<float>(radius), color);
}

void Graphics::DrawRect(const Vec2 &pos, const int width, const int height, const Color &color)
{
	RLAPI::DrawRectangleLines(static_cast<int>(pos.x), static_cast<int>(pos.y), width, height, color);
}

void Graphics::DrawFillRect(const Vec2 &pos, const int width, const int height, const Color &color)
{
	RLAPI::DrawRectangle(static_cast<int>(pos.x), static_cast<int>(pos.y), width, height, color);
}
...
void Graphics::DrawFillPolygon(const Vec2 &pos, const std::vector<Vec2> &vertices, const Color &color)
{
	...
	{
		Vector2 rayVertex;
		rayVertex.x = vertex.x;
		rayVertex.y = vertex.y;
		rayVertices.push_back(rayVertex);
	}

	RLAPI::DrawTriangleStrip(rayVertices.data(), static_cast<int>(rayVertices.size()), color);
	RLAPI::DrawCircle(static_cast<int>(pos.x), static_cast<int>(pos.y), 1.0f, BLACK);
}
...
void Graphics::DrawTexture(const Vec2 &pos, const float width, const float height, const float rotation, const Texture2D *texture)
{
	...
	const Rectangle destRect = {pos.x - (width / 2), pos.y - (height / 2), static_cast<float>(width), static_cast<float>(height)};
	...
	RLAPI::DrawTexturePro(*texture, srcRect, destRect, origin, rotationDeg, BLANK);
}
